...
namespace Cafe_Project
{
    ...
    {
        private CafeItemsRepo _repo = new CafeItemsRepo();
        ...
        private void UpdateItemsbyName()
        {
            ...
            Console.WriteLine("Enter the Meal Name for the content you would like to see.");
            CafeItems itemsToDisplay = _repo.AddCafeItemsMealName(Console.ReadLine());

            if (itemsToDisplay != null)
            {
                Console.WriteLine($"Meal Name: {itemsToDisplay.MealName}\n" +
                    $"Meal Descritpion: {itemsToDisplay.MealDescription}\n" +
                    $"Meal Ingredients: {itemsToDisplay.MealIngredients}\n" +
                    $"Meal Cost: {itemsToDisplay.MealCost}\n" +
                    $"Meal Number: {itemsToDisplay.MealNumber}");
            }
            else
            {
                Console.WriteLine("There is no content by that name");
            }
        }
        private void UpdateCafeItems()
        {
            ...
            if (wasUpdated)
            {
                Console.WriteLine("The cafe item was updated successfully.");
            }
            else
            {
        ...
        private void SeedCafeItemsList()
        {
            ...
            CafeItems theMarcia = new CafeItems("Spicy Burger", "Single Patty on a Pretzel Bun with sauce with a side of seasoned fries and a regular drink", "Bun, Lettuce, Tomatoe, Pickle, Cheddar Cheese", 9.99m, 1);
